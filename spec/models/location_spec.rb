...
RSpec.describe Location, type: :model do
  ...
  describe '#available_slots' do
    ...
    context 'when there are no other appointments booked for the day' do
      it 'starts the next possible slot at the beginning of the day' do
        expected_available_slots = location.available_slots(Date.today, Date.today)
        expected_next_slot_start = expected_available_slots.first[:from] unless expected_available_slots.first.nil?
    ...
    context 'when there are multiple booked slots during a day' do
      ...
      it 'finds convenient slots around all of them and around the lunch break' do
        ...
        expect(expected_available_slots).to eq([
    ...
    context 'when a booked appointment is after lunch' do
      ...
      context 'and there is not enough time after it for a possible slot before day end' do
        ...
        it 'does not identify any more possible slots for that day' do
          ...
          expect(expected_available_slots.size).to eq(0)
  ...
  describe '#unavailable_slots' do
    ...
    context 'when there are no other appointments booked for the day' do
      it 'only returns the lunch break' do
        expected_unavailable_slots = location.unavailable_slots(Date.today, Date.today)
        expected_next_slot_start = expected_unavailable_slots.first[:from] unless expected_unavailable_slots.first.nil?

        expect(expected_unavailable_slots).to eq([
          { from: DateTime.parse("12pm"), to: DateTime.parse("1pm") }
        ])
      end
    end

    context 'when there is one appointment booked before lunch' do
      let(:existing_appointment1) { create(:appointment,
        location_id: location.id,
        from: DateTime.parse("10am"),
        to: DateTime.parse("11:30am"))
      ...
      it 'returns the morning appointment and the lunch break' do
        existing_appointment1.reload
        expected_unavailable_slots = location.unavailable_slots(Date.today, Date.today)

        expect(expected_unavailable_slots).to eq([
          { from: existing_appointment1.from, to: existing_appointment1.to },
          { from: DateTime.parse("12pm"), to: DateTime.parse("1pm") }
        ])
      end
    end

    context 'when there is one appointment booked after lunch' do
      let(:existing_appointment2) { create(:appointment,
        location_id: location.id,
        from: DateTime.parse("2pm"),
        to: DateTime.parse("4pm"))
      ...
      it 'returns the afternoon appointment and the lunch break' do
        existing_appointment2.reload
        expected_unavailable_slots = location.unavailable_slots(Date.today, Date.today)

        expect(expected_unavailable_slots).to eq([
          { from: DateTime.parse("12pm"), to: DateTime.parse("1pm") },
          { from: existing_appointment2.from, to: existing_appointment2.to }
        ])
      end
    end

    context 'when there is one morning appointment and two after lunch' do
      let(:existing_appointment1) { create(:appointment,
        location_id: location.id,
        from: DateTime.parse("10am"),
        to: DateTime.parse("11:30am"))
      ...
      it 'returns the afternoon appointment and the lunch break' do
        ...
        expect(expected_unavailable_slots).to eq([
          ...
          { from: DateTime.parse("12pm"), to: DateTime.parse("1pm") },
          { from: existing_appointment2.from, to: existing_appointment2.to },
          { from: existing_appointment3.from, to: existing_appointment3.to }
        ])
      end
    end
  end
